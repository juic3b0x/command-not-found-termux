...
dbpath = "/data/data/com.termux/files/usr/var/lib/command-not-found/commands.db"
...
class CommandNotFound(object):
    ...
    prefixes = (
        ...
        "/data/data/com.termux/files/usr/local/sbin",
        "/data/data/com.termux/files/usr/games")

    snap_cmd = "/data/data/com.termux/files/usr/bin/snap"

    output_fd = sys.stderr

    def __init__(self, data_dir="/data/data/com.termux/files/usr/share/command-not-found"):
    ...
    def _getSourcesList(self):
        ...
        # /data/data/com.termux/files/usr/share/python-apt/templates/
        ...
        if package_name:
            ...
            if answer.lower() == _("y"):
                if self.euid == 10438:
                    ...
                    command_prefix = ""
                install_command = "%spkg install %s" % (command_prefix, package_name)
    ...
    def print_spelling_suggestions(self, word, mispell_packages, mispell_snaps, max_alt=15):
        ...
        if len(mispell_snaps) > 0:
            ...
            if self.user_can_sudo:
                print(_("Try: %s <deb name>") % "pkg install", file=self.output_fd)
            else:
                print(_("Try: %s <deb name>") % "pkg install", file=self.output_fd)
        self.output_fd.flush()
    ...
    def advice_multi_deb_package(self, command, packages, snaps):
        ...
        for i, package in enumerate(packages):
            ...
            if package[2] in self.sources_list:
                print("%spkg install %-*s%s" % (self.sudo(), pad, package[0], ver), file=self.output_fd)
            else:
                print("%spkg install %-*s%s" % (self.sudo(), pad, package[0], ver) + " (" + _("You will have to enable component called '%s'") % package[2] + ")", file=self.output_fd)
    ...
    def advise(self, command, ignore_installed=False):
        ...
        if not (os.path.exists("/data/data/com.termux/files/usr/bin/apt") or
                os.path.exists("/data/data/com.termux/files/usr/bin/aptitude")):
            return False
